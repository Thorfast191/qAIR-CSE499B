...
- **`destructive_fraction` saturates by construction and is not evidence of working interference.** With `K = 2N` matched support/attack pairs it is a step function of the single scalar `full_model.polarity_phase` — 0.000 up to 1.6 rad, 0.494 at 2.4, 0.994 at 2.8, 1.000 from 3.0 up — and that scalar is initialized at π, past the step, so it reads 1.0 for an untrained model. It also cannot report the sign of the effect. Cite `coherent_acc` vs `classical_acc` instead: the same amplitudes and energies ranked coherently versus decohered, from one forward pass, with the LLM prior excluded from both.
